...
```
node dist-bench/bench/tool-cli.js search "질의" [limit] --no-vocab-gap
node dist-bench/bench/tool-cli.js article <law_id> <article_no>
```

먼저 `node dist-bench/bench/tool-cli.js search "기부금" 1 --no-vocab-gap` 로 도구가 도는지 확인하라.
...
2. 결과가 질문과 안 맞아 보이면 질의를 바꿔 다시 검색하라.
