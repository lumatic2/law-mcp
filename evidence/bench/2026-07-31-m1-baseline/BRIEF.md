# 블라인드 법령 조사 과제 (M1 before 기준선 측정용)
...
- **`bench/corpus.json` · `bench/contexts-tf2.json` 과, 이 디렉터리(`evidence/bench/2026-07-31-m1-baseline/`)
  밖의 `evidence/` · `plans/` · `changesets/` · `archive/` · `research/` · `docs/` · `ROADMAP.md` 를
  절대 열지 마라.** 정답이 거기 있다.
- 이 디렉터리의 `BRIEF.md`·`tasks.json`·자기 run 파일과 위 두 CLI 외에 레포 파일을 읽지 마라.
  코드를 수정하지 마라.

## 산출물

이 디렉터리(`evidence/bench/2026-07-31-m1-baseline/`)에 자기 번호의 run 파일을 쓴다
(blind-1 → `run1.json`, blind-2 → `run2.json`, blind-3 → `run3.json`):

```json
{"tool_ok":true,"agent":"blind-1","results":[
  {"case_id":"v2-01","turns":3,"law_name":"...","article_no":"제N조","not_found":false,
   "queries":["실제로 던진 질의들"],"saw_vocab_warning":true}
]}
```

**43건 전부** 처리한다. 끝나면 파일 경로만 한 줄로 보고하라.
