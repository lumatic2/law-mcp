...
- **`bench/corpus.json` · `bench/contexts-tf2.json` 과, 이 디렉터리(`evidence/bench/2026-08-01-m7-expanded/`)
...
이 디렉터리(`evidence/bench/2026-08-01-m7-expanded/`)에 자기 번호의 run 파일을 쓴다
(blind-1 → `run1.json`, blind-2 → `run2.json`, blind-3 → `run3.json`):

```json
{"tool_ok":true,"agent":"blind-1","results":[
  {"case_id":"exp-02","turns":3,"law_name":"...","article_no":"제N조","not_found":false,
   "queries":["실제로 던진 질의들"],"saw_vocab_warning":true}
]}
```

**40건 전부** 처리한다. 끝나면 파일 경로만 한 줄로 보고하라.
