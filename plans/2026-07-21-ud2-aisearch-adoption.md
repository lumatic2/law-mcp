...
- **선행 조건 (UD1 F12)**: 착수 전 **휴지 후 기준선 재측정**으로 상류 쓰로틀링 여부를 가린다.
  48% 가 회복되지 않으면 그게 정상 기준선이고, 회복되면 쓰로틀 상태에서 잰 A/B 는 무효다.
- **중단점**: blocked / error / decision_required / risk_gate(실 API rate limit) /
  **dev A/B 에서 순 이득 <3건 = 이득 없음**(가설 기각 후 정지·기록) /
...
**② 자기선언 도메인**
- **판정 방식 (UD1 실측으로 개정 — F13)**: 계획이 세웠던 **2σ 문턱은 폐기한다.** UD1 에서 11회를
  재보니 문제는 노이즈가 아니라 **드리프트**였다(60% 6회 연속 → 48% 10회 연속, 안 돌아옴).
  연속 실행의 σ 는 드리프트를 못 잡고(한 배치는 σ=0.0%p → 문턱 0 = 무의미), 더 나쁘게는 A/B 를
  순차로 재면 드리프트가 통째로 **나중에 잰 쪽의 불이익**이 된다. 대신:
    ① **교차 측정 필수** — A 배치 → B 배치가 아니라 **쿼리마다 A·B 를 연달아** 잰다.
    ② **판정은 쿼리 단위 승패**로 한다 — 같은 시점·같은 쿼리에서 어느 쪽이 맞았나. 드리프트 무관.
    ③ **채택 조건 = 새로 깨지는 쿼리 0 + 순 이득 ≥3건**(dev 25건 기준).
    ④ 절대 수치를 인용할 땐 **측정 시각과 그 세션의 누적 호출량**을 함께 적는다.
...
- [ ] **step-2** candidate-merge — 후보 생성 편입 + dev A/B
  ...
  - Dependencies: step-1
  - Verify: **교차 측정**(쿼리마다 A·B 연달아)으로 dev 25건 A/B — 채택은 **새로 깨지는 쿼리 0
    AND 순 이득 ≥3건** 일 때만(F13). 쿼리 단위 승패 표를 evidence 에 첨부
...
- **DoD**: ① 교차 측정 dev A/B 에서 `aiSearch` 병합의 **순 이득 ≥3건** ② **새로 깨지는 쿼리 0**
...
- dev A/B 순 이득 <3건 → 채택 중단, 기각 기록 후 정지·보고.
...
- F6: `aiRltLs`(AI 연관법령)·`lsRlt`(관련법령 그래프)는 같은 계열의 미사용 API — UD2 결과에 따라
  후속 후보.
- F12(상속·최우선): 상류 쓰로틀링 의심. 사실이면 벤치가 아니라 **실사용자 문제**다.
- **하류 회귀 범위(UD0 교훈)**: 순위를 바꾸는 변경은 그 순위를 *입력으로 쓰는 도구*까지 회귀
  범위에 넣는다. LB5 가 이걸 안 해서 `get_law_article` 이 다른 법을 가리켰다.

## 진행 로그 (append-only)

- 2026-07-21 plan 작성 — horizon `upstream-delivery` 개설과 함께 일괄 작성(CS6 horizon-run).
- 2026-07-21 UD1 실측으로 판정 규약 개정(승인 등록 **전**) — 2σ 문턱 폐기, 교차 측정 + 쿼리 단위
  승패로 대체(F13). 착수 선행 조건으로 휴지 후 기준선 재측정 추가(F12).
