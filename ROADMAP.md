...
- **행정 도메인** — 유일하게 재현되는 약점(dev 60% / holdout 67%). **2026-07-21 진단 완료**
  → `evidence/bench/2026-07-21-admin-domain-diagnosis.md`. 순위 문제가 아니라 **도달 불가** 문제다:
  실패 3건 모두 정답이 행정 총칙법(행정기본법·행정절차법)이고, upstream 의 어느 채널도 주지 않는다
  (aiSearch 20위·aiRltLs 10건·용어연계 0건 전부 없음, 본문검색은 613건 가나다순 꼬리).
  **milestone-grade** — 새 평가 세트 + 조문제목 신호 설계 + A/B 가 한 묶음이다.
- ~~**F6 관계 그래프**~~ — 위 진단에서 **재료 아님으로 확정**. `aiRltLs` 는 우리와 똑같은 오답을
  주고 `lsRlt` 는 빈 응답이다. 이 용도로는 백로그에서 내린다.
- ~~**F20 이름 해석 부분문자열 오집**~~ — **2026-07-21 수리 완료**
  (`changesets/20260721-f20-accidental-name-match`). 경고 → 거절.
- 새 평가 세트 라벨링 (품질 판정을 계속하려면 선행 — 위 행정 건의 전제조건이기도 하다)
