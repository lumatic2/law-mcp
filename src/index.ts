...
/**
 * 소비 에이전트에게 **세션당 한 번** 전달되는 사용 지침 (AR3).
 *
 * 여기 적는 것은 도구 설명에 넣기엔 반복이고, 그렇다고 안 알리면 에이전트가 첫 결과를
 * 그대로 답으로 쓰는 것들이다. 실측 근거: 일상어 단발 질의는 정확 도달 0/5 였고,
 * 같은 쟁점을 법률 용어로 바꿔 재질의하면 3/5 로 복구됐다(2026-07-22~23).
 * 즉 **재질의 여부가 결과를 가른다** — 그걸 아는 것은 에이전트뿐이므로 여기서 말한다.
 */
const INSTRUCTIONS = [
  "한국 법령·판례·행정규칙 조회 도구다. 다음을 지켜야 답이 맞는다.",
  "",
  "1. **한 번 검색하고 끝내지 마라.** 이 도구는 상위 결과가 관련도순이 아닐 수 있고,",
  "   응답의 `warnings` 가 그 사실을 매번 알려 준다. warnings 를 읽고 필요하면 재질의하라.",
  "2. **일상어로 물으면 정답 법령이 후보에 아예 안 들어온다.** 어휘 공백 경고가 뜨면",
  "   질의를 *좁히지 말고* 법률 용어로 **바꿔서** 다시 검색하라 — 좁히기는 같은 어휘",
  "   안에서 돌아 이 실패를 못 푼다.",
  "3. **조문은 반드시 본문을 확인하고 인용하라.** 검색 결과의 조문 목록만 보고 답하지 마라.",
  "   `get_law_article` 로 본문을 받아 확인한다.",
  "4. **세금·연도가 걸린 질문은 `as_of` 를 쓴다.** 같은 조문이 귀속연도에 따라 다르다.",
  "   생략하면 현행이 오고, 과거 연도 질문에는 틀린 답이 된다.",
  "5. **못 찾으면 못 찾았다고 답하라.** 비슷해 보이는 다른 분야의 조문을 지목하지 마라 —",
  "   법령 검색은 분야가 다르면 문언이 비슷해도 완전히 틀린 답이다.",
].join("\n");
